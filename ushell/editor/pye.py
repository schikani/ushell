...
class Editor:
    KEYMAP = {  ## Gets lengthy
        ...
        "\x03": KEY_COPY,  ## Ctrl-C
        "\r": KEY_ENTER,
        "\x08": KEY_BACKSPACE,  ## Ctrl-? (8) # Tried on ESP32
        "\x1b[3~": KEY_DELETE,
    ...
    def handle_edit_keys(self, key, char):  ## keys which change content
        ...
        if key == KEY_DOWN:
            ...
            if self.mark is None:
                ...
                self.undo_add(lrange[0], self.content[lrange[0]:lrange[1]], KEY_INDENT,
                              lrange[1] - lrange[0])  ## undo replaces
                for i in range(lrange[0], lrange[1]):
                    if len(self.content[i]) > 0:
                        self.content[i] = ' ' * (self.tab_size - self.spaces(self.content[i]) % self.tab_size) + \
                                          self.content[i]
            ...
            if self.mark is None:
                ...
                lrange = self.line_range()
                self.undo_add(lrange[0], self.content[lrange[0]:lrange[1]], KEY_DEDENT,
                              lrange[1] - lrange[0])  ## undo replaces
                for i in range(lrange[0], lrange[1]):
                    ...
                    while True:  ## and go
                        ...
                        if ni is not None and (self.cur_line != (end_line - 1) or self.col < end_col):  ## Pattern found
                            ...
                            if q == 'q' or key == KEY_QUIT:
                                ...
                                self.content[self.cur_line] = self.content[self.cur_line][:self.col] + rpat + \
            ...
            if self.mark is None:
                ...
                lrange = self.line_range()
            self.undo_add(lrange[0], self.content[lrange[0]:lrange[1]], KEY_COMMENT,
                          lrange[1] - lrange[0])  ## undo replaces
